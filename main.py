...
for i in range(44, 48):
    marcos.append({"num_pagina": i,"tamano": 5, "proceso": "SO", "espacio_ocupado": 5, "estado": 4})
    memoria -= 5


def imprimir_marcos():
    print("Marco     Espacio Ocupado         Proceso           Marco     Espacio Ocupado         Proceso")
    for i in range(0, 47, 2):
        print(f"{i}                {marcos[i]['espacio_ocupado']}/5               {marcos[i]['proceso']}              {i+1}                {marcos[i+1]['espacio_ocupado']}/5               {marcos[i+1]['proceso']}")
def ubicar_proceso(proceso):
    marcotes = []
    global memoria
    for pagina in proceso['paginas']:
        for i in range(0, 48):
            if (marcos[i]['proceso'] == "Libre" and i not in marcotes):
                marcotes.append(i)
                break
    if (len(proceso['paginas']) != len(marcotes)): return False
    for i in range(0, len(proceso['paginas'])):
        marcos[marcotes[i]]['proceso'] = proceso['id']
        marcos[marcotes[i]]['espacio_ocupado'] = proceso['paginas'][i]['tamano']
        proceso['paginas'][i]['marco'] = marcotes[i]
        marcos[marcotes[i]]['estado'] = 1
        memoria -= 5
    return True
...
def liberar_memoria(proceso):
    global memoria
    for pagina in proceso['paginas']:
        marcos[int(pagina['marco'])]['proceso'] = "Libre"
        marcos[int(pagina['marco'])]['espacio_ocupado'] = 0
        memoria += 5
        ...
        if self.entrada_variable.get() == "" or eval(self.entrada_variable.get() + " != 0") == False or self.entrada_variable2.get() == "" or eval(self.entrada_variable2.get() + " != 0") == False:
            ...
            for i in range(n_procesos):
                ...
                while (aux != 0):
                    if (aux == 0): break
                    if (aux - 5 >= 0):
                        paginas.append({"tamano": 5, "marco": None})
                        aux -= 5
                    else: 
                        paginas.append({"tamano": aux, "marco": None})
                        aux = 0
                        break
...
class Procesos_Nuevos(ctk.CTkFrame):
    ...
    def agregar_nuevo_proceso(self):
        ...
        if len(procesos) == 1 and memoria < procesos[0]['tamano']:
            self.master.memory.actualizar()
...
class MemoryFrame(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)
        #Etiqueta para el nuevo proceso
        label = ctk.CTkLabel(self, text="Sig. Proceso:  ", padx=5, pady=5, font=("Arial", 16, "bold"))
        label.grid(row=0, column=0, padx=10, pady=5, sticky="w")
        self.proceso = ctk.CTkLabel(self, text="N/A", padx=5, pady=5, font=("Arial", 16, "bold"))
        self.proceso.grid(row=0, column=1, padx=10, pady=5)
        self.memoria = ctk.CTkLabel(self, text="N/A", padx=5, pady=5, font=("Arial", 16, "bold"))
        self.memoria.grid(row=0, column=2, padx=10, pady=5)
        label_marcos = ctk.CTkLabel(self, text="Marcos disp.:  ", padx=5, pady=5, font=("Arial", 16, "bold"))
        label_marcos.grid(row=1, column=0, padx=10, pady=5, sticky="w")
        self.disponibles = ctk.CTkLabel(self, text="N/A", padx=5, pady=5, font=("Arial", 16, "bold"))
        self.disponibles.grid(row=1, column=1, padx=10, pady=5)
        self.actualizar()
    def actualizar(self):
        if len(procesos) > 0:
            self.proceso.configure(text=f"{procesos[0]['id']}")
            self.memoria.configure(text=f"{procesos[0]['tamano']}")
        else:
            self.proceso.configure(text="N/A")
            self.memoria.configure(text="N/A")         
    def actualizar_marcos(self):
        self.disponibles.configure(text=f"{int(memoria / 5)}") 

class Procesos_Listos(ctk.CTkScrollableFrame): #Tabla de procesos nuevos
    def __init__(self, master, root, **kwargs):
        ...
        self.procesos_nuevos = root.nuevos
    ...
    def crear_labels(self):
        ...
        ubicar_proceso(self.proceso_inicial)             
        uwu = 0
        for i in range(0, len(procesos)):
            
            if (ubicar_proceso(procesos[i]) == True):
                uwu += 1
                self.procesos_listos.append(procesos[i])
                self.root.actualizar_estado_proceso(procesos[i]['id'], 1) #Cambiar estado a listo
            else: break
        for j in range(0, uwu):
            procesos.pop(0)
        ...
        self.root.memory.actualizar()
        self.root.memory.actualizar_marcos()
    ...
    def encolar_proceso(self, proceso):
        for pagina in proceso['paginas']:
            marcos[pagina['marco']]['estado'] = 1
        self.procesos_listos.append(proceso)
        self.root.actualizar_estado_proceso(proceso['id'], 1) #Cambiar estado a listo
    ...
    def limpiar_label(self):
        self.actualizar_lista()
        for i in range(len(self.procesos_listos), 25):
            self.labels_procesos[i][0].configure(text="")
            self.labels_procesos[i][1].configure(text="")
            self.labels_procesos[i][2].configure(text="")


class Procesos_Bloqueados(ctk.CTkScrollableFrame): #Tabla de procesos nuevos
    def __init__(self, master, root, **kwargs):
        super().__init__(master, **kwargs)
        self.root = root
        self.grid_columnconfigure(0, weight=1)
        self.grid_columnconfigure(1, weight=1)
        self.procesos_listos = self.root.proceso_pend
    ...
    def proceso_bloqueado(self, proceso):
        ...
        self.root.actualizar_estado_proceso(proceso['id'], 2) #Cambiar estado a bloqueado
        self.actualizar_lista()
    def actualizar_lista(self):
        i = 0
    ...
    def limpiar_label(self):
        self.actualizar_lista()
        for i in range(len(self.procesos_bloqueados), 25):
            self.labels_procesos[i][0].configure(text="")
            self.labels_procesos[i][1].configure(text="")
    def conteo(self):
...
class Procesos_Ejecucion(ctk.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        label_tamano = ctk.CTkLabel(self, text="Tamaño", padx=5, pady=5, font=("Arial", 12, "bold"), justify="center")
        label_tamano.grid(row=7, column=0, padx=5, pady=5)
        label_paginas = ctk.CTkLabel(self, text="N. Páginas", padx=5, pady=5, font=("Arial", 12, "bold"), justify="center")
        label_paginas.grid(row=8, column=0, padx=5, pady=5)
        ...
        self.texto_tamano = ctk.CTkLabel(self, text="", padx=5, pady=5, font=("Arial", 13))
        self.texto_tamano.grid(row=7, column=1, padx=5, pady=5)
        self.texto_paginas = ctk.CTkLabel(self, text="", padx=5, pady=5, font=("Arial", 13))
        self.texto_paginas.grid(row=8, column=1, padx=5, pady=5)
    ...
    def proceso_en_ejecucion(self, proceso):
        ...
        self.texto_tamano.configure(text=f"{proceso['tamano']}")
        self.texto_paginas.configure(text=f"{len(proceso['paginas'])}")

    def error_proceso(self):
        print(f"memoria antes: {memoria}")
        liberar_memoria(self.proceso_actual)
        self.master.memory.actualizar_marcos()
        print(f"memoria después: {memoria}")
        self.tiempo_quantum = 0
        ...
        if(len(self.procesos_listos.procesos_listos) > 0 and len(procesos) > 0 and memoria < procesos[0]['tamano']): 
            print("entró en condición 1")
            self.nuevo_proceso()
        elif(len(procesos) > 0 and len(self.procesos_listos.procesos_listos) == 0 and memoria >= procesos[0]['tamano']):
            print("entró en condición 2")
            self.agregar_nuevos_a_listos()
            self.nuevo_proceso()
        else: 
            print("entró en condición 3")
            self.tiempo_restante = 0
            self.limpiar_datos()
    def interrumpir_proceso(self):
        ...
        for pagina in self.proceso_actual['paginas']:
            marcos[pagina['marco']]['estado'] = 3
    ...
    def nuevo_proceso(self):
        self.tiempo_quantum = 0
        self.texto_tq.configure(text=f"{self.tiempo_quantum}")
        if(len(procesos) > 0 and ubicar_proceso(procesos[0]) == True):
    ...
    def agregar_nuevos_a_listos(self):
        if ubicar_proceso(procesos[0]) == True:
    ...
    def limpiar_datos(self):
        ...
        self.texto_tamano.configure(text="")
        self.texto_paginas.configure(text="")
    ...
    def avanzar_tiempo(self):
        if (self.ejecutar == False or self.proceso_actual == None): return
            ...
            if (self.tiempo_restante == 0): #Si el tiempo restante es 0, se hace lo siguiente:
                ...
                if (len(self.procesos_listos.procesos_listos) == 0 and len(procesos) == 0): 
                    self.limpiar_datos()
                    return
                self.nuevo_proceso()
                #self.tiempo_quantum = 0
...
#Clase para crear la tabla de memoria
class Tabla_memoria(ctk.CTkToplevel):
    def __init__(self, master, **kwargs):
        super().__init__(master, **kwargs)   
        global colores
        global marcos
        self.title("Tabla de Memoria")
        self.resizable(False, False)
        self.marcos = []
        self.crear_headers()
        self.crear_ventana()
    ...
    def actualizar(self):
        for i, marco in enumerate(self.marcos):
            if(marco[1].cget("text") != marcos[i]['proceso']):
                for col in range(5):
                    color = colores[0]  
                    if col < marcos[i]["espacio_ocupado"]:
                        color = colores.get(marcos[i]["estado"], "gray90")
                    marco[0][col].configure(fg_color=color)
                marco[1].configure(text=f"{marcos[i]['proceso']}")
        self.after(1000, self.actualizar)

    def crear_headers(self):
        header1 = ctk.CTkLabel(self, text="No.\nMarco", anchor="w", font=("Arial", 14, "bold"))
        header1.grid(row=0, column=0, padx=5, pady=5)

        header2 = ctk.CTkLabel(self, text="Proceso", anchor="w", font=("Arial", 14, "bold"))
        header2.grid(row=0, column=2, padx=5, pady=5)

        header3 = ctk.CTkLabel(self, text="No.\nMarco", anchor="w", font=("Arial", 14, "bold"))
        header3.grid(row=0, column=3, padx=5, pady=5)

        header2 = ctk.CTkLabel(self, text="Proceso", anchor="w", font=("Arial", 14, "bold"))
        header2.grid(row=0, column=5, padx=5, pady=5)

    def crear_cuadros(self, parent, columnas=5, width=100, height=20, marco=None):
        box = ctk.CTkFrame(parent, width=width, height=height)
        box.grid_propagate(False) 

        for col in range(columnas):
            color = colores[0]  
            if marco and col < marco["espacio_ocupado"]:
                color = colores.get(marco["estado"], "gray90")
    ...
    def crear_ventana(self):
        numeros_par = [marco for marco in marcos if marco["num_pagina"] % 2 == 0]
        numeros_impar = [marco for marco in marcos if marco["num_pagina"] % 2 != 0]

        for row in range(len(numeros_par)):
            if row > 0:
                separator = ctk.CTkFrame(self, height=1, fg_color="gray50")
                separator.grid(row=row * 2, column=0, columnspan=6, sticky="ew")

            # Números pares
            even_label = ctk.CTkLabel(self, text=str(numeros_par[row]["num_pagina"]), anchor="center")
            even_label.grid(row=row * 2 + 1, column=0, padx=5, pady=0)

            divided_box1 = self.crear_cuadros(self, columnas=5, width=100, height=20, marco=numeros_par[row])
            divided_box1.grid(row=row * 2 + 1, column=1, padx=5, pady=0)
            even_proceso_label = ctk.CTkLabel(self, text=str(numeros_par[row]['proceso']), anchor="center")
            even_proceso_label.grid(row=row * 2 + 1, column=2, padx=5, pady=0)

            self.marcos.append((divided_box1.winfo_children(), even_proceso_label))

            # Números impares
            if row < len(numeros_impar):  # Para evitar índices fuera de rango
                odd_label = ctk.CTkLabel(self, text=str(numeros_impar[row]["num_pagina"]), anchor="center")
                odd_label.grid(row=row * 2 + 1, column=3, padx=5, pady=0)

                divided_box2 = self.crear_cuadros(self, columnas=5, width=100, height=20, marco=numeros_impar[row])
                divided_box2.grid(row=row * 2 + 1, column=4, padx=5, pady=0)

                odd_proceso_label = ctk.CTkLabel(self, text=str(numeros_impar[row]['proceso']), anchor="center")
                odd_proceso_label.grid(row=row * 2 + 1, column=5, padx=5, pady=0)

                self.marcos.append((divided_box2.winfo_children(), odd_proceso_label))
...
class Contador(ctk.CTkFrame):
    ...
    def actualizar_tiempo(self):
        if self.contar == True and self.master.terminar == False:  #Siempre que contar sea True, cuenta
            if(self.procesos_ejecucion.proceso_actual == None and memoria == 220): 
                self.terminar()
                return            
            self.tiempo_total += 1
...
class Aplicacion(ctk.CTk):
    def __init__(self):
        ...
        #Creación del cuadro del siguiente proceso
        self.memory = MemoryFrame(master=self)
        self.memory.grid(row=0, column=2, padx=30, pady=20, sticky="nw")
        #Creación de la tabla de procesos listos
        self.proceso_pend = Procesos_Listos(master=self, root=self)
        self.proceso_pend.grid(row=1, column=0, padx=20, pady=10, sticky="nsew")
        #Creación de la tabla de procesos bloqueados
        self.proceso_bloq = Procesos_Bloqueados(master=self, root=self)
        self.proceso_bloq.grid(row=2, column=0, padx=20, pady=10, sticky="nsew")
        ...
        self.contador.grid(row=2, column=2, padx=20, pady=10, sticky="ne")
    ...
    def abrir_BCP(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = Tabla_BCP(self)  
        else:
            self.toplevel_window.focus() 

    def abrir_BCP_previa(self):
        if self.toplevel_window is None or not self.toplevel_window.winfo_exists():
            self.toplevel_window = Tabla_BCP_previa(self)  
        else:
            self.toplevel_window.focus() 
    ...
    def key_pressed(self, event):
        ...
        if(tecla == "t" and self.ejecutar == True and not self.terminar):
            self.ejecutar = False
            self.proceso_eje.ejecutar = False
            self.contador.contar = False
            self.after(1000, self.bandera()) 
